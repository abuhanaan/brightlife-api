...
public class ControlledSubstanceService {
  ...
  public Success submitControlled(ControlledSubstanceForm request){
    ...
    controlledSubstanceForm.setPatientSignDate(request.getPatientSignDate() != null ? Timestamp.from(request.getPatientSignDate().toInstant()) : null);
    controlledSubstanceForm.setGuardianName(request.getGuardianName());
    controlledSubstanceForm.setPatientGuardianRelationship(request.getPatientGuardianRelationship());
    controlledSubstanceForm.setGuardianSignDate(request.getGuardianSignDate() != null ? Timestamp.from(request.getGuardianSignDate().toInstant()) : null);
  ...
  public ControlledSubstanceForm getControlledSubstance(Long id){
    ControlledSubstanceFormEntity controlledSubstanceFormEntity = checkIfControlledSubstanceFormExists(id);
    ControlledSubstanceForm controlledSubstanceDto = new ControlledSubstanceForm();
    controlledSubstanceDto.setId(id);
    controlledSubstanceDto.setPatientId(UUID.fromString(controlledSubstanceFormEntity.getPatientId()));
    controlledSubstanceDto.setIsMinor(controlledSubstanceFormEntity.getIsMinor());
    controlledSubstanceDto.setPatientSignDate(controlledSubstanceFormEntity.getPatientSignDate().toInstant().atOffset(ZoneOffset.UTC));
    controlledSubstanceDto.setGuardianName(controlledSubstanceFormEntity.getGuardianName());
    controlledSubstanceDto.setPatientGuardianRelationship(controlledSubstanceFormEntity.getPatientGuardianRelationship());
    controlledSubstanceDto.setGuardianSignDate(controlledSubstanceFormEntity.getGuardianSignDate().toInstant().atOffset(ZoneOffset.UTC));
  ...
  private ControlledSubstanceFormEntity checkIfControlledSubstanceFormExists(Long id) {
    return controlledSubstanceRepository.findOneById(id).orElseThrow(() ->
        new NotFoundException(String.format("Controlled Substance form with id %s does not exist", id)));
  }

  private void checkForControlledSubstanceUniqueness(String patientId){
    ControlledSubstanceFormEntity controlledSubstanceFormEntity = controlledSubstanceRepository.findOneByPatientId(patientId);
    if (controlledSubstanceFormEntity != null) {
      throw new ConflictException(String.format("Controlled Substance form has already been filled for patient %s", patientId));
    }
  }
}
